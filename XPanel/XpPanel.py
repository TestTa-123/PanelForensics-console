...
class XpPanel:
    def __init__(self) -> None:
        ...
        self.website_name = ['端口', '根目录', '运行目录', '网站名', 'PHP版本', '备注', '规则文件', 'WAF日志',
                             '访问日志']

    def set_path(self) -> None:
        self.db_path = '/usr/local/phpstudy/system/depends/libc.so.1.0.1'
        dec = DecHelper()
        if dec.decrypt_xp_db(self.db_path) == 1:
            self.db_path = '/usr/local/phpstudy/xp.db'
        self.install_path = '/usr/local/phpstudy/install.result'
