...
class OnePanel:
    def __init__(self) -> None:
        ...
        self.installed_app_name = ['创建时间', '修改时间', '应用名', '版本', '参数', '环境配置', 'Compose配置',
                                   '运行状态', '描述', '消息', '容器名称', '服务名称', 'HTTP端口号', 'HTTPS端口号']
        ...
        self.cronjobs_name = ['创建时间', '修改时间', '任务名', '类型', '执行周期', '脚本', '网站名', '数据库', 'URL',
                              '源目录', '执行规则', '是否保存本地', '任务状态', '容器名称']
        ...
        self.websites_name = ['创建时间', '修改时间', '协议', '主域名', '类型', '代号', '备注', '状态', '过期时间',
                              '网站根目录']
    ...
    def set_path(self) -> None:
        self.db_path = '/opt/1panel/db/1Panel.db'
